...
"""
credit: https://github.com/Voxel-Fox-Ltd/Apple.Py/blob/master/cogs/timezone_info.py
"""
...
class TimezoneInfo(utils.Cog):
    ...
    async def settime(self, ctx: utils.Context, *, offset: str = None):
        ...
        match = re.search(
            r"(?P<hour>\d?\d)[:-]?(?P<minute>\d\d) ?(?P<daytime>(?:AM)|(?:PM))?", offset, re.IGNORECASE)
        if not match:
        ...
        # Work how far off we are of the exact time
        now = dt.utcnow()
        ...
        await ctx.send(f"Your time is estimated to be **UTC{hour_offset:=+03d}:{minute_offset:=02d}** - I've stored this in the database.")
    ...
    async def timezone_get(self, ctx: utils.Context, user: discord.Member = None):
        ...
        formatted_time = (
            dt.utcnow() + timedelta(minutes=rows[0]['timezone_offset'])).strftime('%-I:%M %p')
